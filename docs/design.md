...
| Map scene, battle scene, hub scene, scene runner | Game | Core state, content from the Game assembly, atlas, edge files, string table | screen, intents | Medium. Cosmetic by design (D-106, D-111, D-114, D-501). Player text reaches the screen through the text helper (D-499). The content bytes come from the Game assembly (D-508) |
...
| Atlas tool, normal maps, PNG code, PNG import, audio synthesizer | Tools | grids, palette, edited PNG files, tracker rows, parameter files | atlas PNG, normal-map atlas, grids, rendered audio, hash list | Medium. The atlas and the normal-map atlas are committed with a pixel test (D-107, D-184, F-19), and the audio renders at build against its hash list (D-432). Integer math gives the same output on every CI leg (D-502) |
