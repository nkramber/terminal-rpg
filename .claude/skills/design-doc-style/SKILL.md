...
2. **Lessons learned.** Numbered. Each lesson names the event that taught it. Carry lessons from the two reference repositories when they apply.
...
9. **Open questions.** A link to `docs/questions.md` (D-19). The register there has the numbers. Record the date and the answer there when one arrives.
...
A focused roadmap covers one phase or one area, for example combat or the lesson system. It uses the status header and the same sections 1, 5, 7, 8, and 9. It links to the design doc for the system map and the cost model. Its PR-# ids continue the global sequence. It never restates a decision. It cites the D-# id.
